...
const Sidebar = () => {
    ...
    const { data } = useQuery('currentUser', getCurrentUser, { retry: 0 })
    const [isAdmin, setIsAdmin] = useState(false)

    const navs = [
        { admin:false, name: 'Dashboard', icon: { icon: 'material-symbols:dashboard-outline' }, path: '' },
    ...
    return (
        <ul className={'flex flex-col gap-1 text-[20px] text-secondary-2'}>
            ...
            {isAdmin && <li className="relative">
                <button id="dropdownDefaultButton" className={'disabled flex gap-3 rounded-full px-4 pt-3 items-center'}>
